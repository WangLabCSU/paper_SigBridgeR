...
scpas_random_search <- cbind(
  scpas_random_search,
  data.table::fread(paste0("scpas/3_scpas_random_search.csv")),
  data.table::fread(paste0("scpas/4_scpas_random_search.csv")),
  data.table::fread(paste0("scpas/5_scpas_random_search.csv"))
)
...
nms <- colnames(scpas_random_search)

# 第3组：位置 22–31  → process_31 ... process_310
nms[22:31] <- paste0("process_3", 1:10)

# 第4组：位置 32–41  → process_41 ... process_410
nms[32:41] <- paste0("process_4", 1:10)

# 第5组：位置 42–51  → process_51 ... process_510
nms[42:51] <- paste0("process_5", 1:10)

colnames(scpas_random_search) <- nms

scpas_random_search[, "process_110"] <- FALSE
scpas_random_search[, "process_23"] <- FALSE
