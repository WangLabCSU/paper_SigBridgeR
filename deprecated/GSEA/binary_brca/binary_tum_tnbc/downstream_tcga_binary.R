...
irgsea_score <- SigBridgeR::MergeResult(irgsea_score, tnbc_tcga_merged)
...
if (file.exists(file.path(data_path, "binary_tnbc_tcga_dge.qs"))) {
  dge_res <- qs::qread(
    file.path(data_path, "binary_tnbc_tcga_dge.qs"),
    nthreads = 4L
  )
