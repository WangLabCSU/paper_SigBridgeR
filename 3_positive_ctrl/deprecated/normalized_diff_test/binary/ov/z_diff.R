...
esmat_root <- "/home/yyx/R/Project/R_code/SigBridgeR/Tmp/ssGSEA_positive_compare/normalized_esmat/binary/ov"
esmat_files <- list.files(esmat_root, recursive = TRUE) %>%
  grep("ssGSEA_score_z\\.qs", ., value = TRUE)

bulks <- gsub(".*(GSE\\d+).*", "\\1", esmat_files) %>% unique()
...
# seurat_GSE140082 <- qs::qread(
#   file.path(seurat_path, "GSE140082_ov_merged_seurat.qs"),
#   nthreads = 4L
# )
# seurat_GSE32062 <- qs::qread(
#   file.path(seurat_path, "GSE32062_soc_merged_seurat.qs"),
#   nthreads = 4L
# )
...
#   bulks,
#   function(bulk_i) {
#     seurat_i <- get(paste0("seurat_", bulk_i))
#     methods <- grep(
...
purrr::walk(
  ...
  function(bulk_i) {
    ...
    file_name <- paste0("z_diff_", bulk_i, "_binary.csv")
