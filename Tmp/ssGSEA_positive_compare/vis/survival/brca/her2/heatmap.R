...
diff_path <- "Tmp/ssGSEA_positive_compare/diff_test/survival/brca/her2"
...
esmat_root <- "Tmp/ssGSEA_positive_compare/esmat/survival/brca/her2"
...
esmat_TCGA_BRCA$survival_TCGA_BRCA_pos_ssGSEA <- 0
...
data.table::fwrite(
  combined,
  file = "Tmp/ssGSEA_positive_compare/vis/survival/brca/her2/survival_her2_test_data.csv"
)

cli::cli_alert_success(
  "Combined test results saved to {.file survival_her2_test_data.csv}"
)
...
purrr::walk(
  ...
  function(ssgsea_type) {
    ...
    p <- ggplot(plot_df2) +
      ...
      labs(
        title = "HER2 BRCA ssGSEA Pos Ctrl",
        subtitle = glue::glue(
          "sc GSE161529 HER2 BRCA ; {ssgsea_type} marker \nsurvival as phenotype\nGSE42568: Tumor/Normal\nGSE162228: Relapse/Non-relapse;\nTCGA_BRCA: Tumor/Normal"
        )
      ) +
      theme(
...
htmap_dir <- "Tmp/ssGSEA_positive_compare/vis/survival/brca/her2/heatmap"
...
purrr::walk(c("pos", "neg"), function(ssgsea_type) {
  file_name <- glue::glue("her2_{ssgsea_type}marker_htmap.png")
