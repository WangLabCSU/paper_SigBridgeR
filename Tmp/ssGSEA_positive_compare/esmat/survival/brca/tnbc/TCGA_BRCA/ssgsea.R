...
setwd(
  "/home/yyx/R/Project/R_code/SigBridgeR/Tmp/ssGSEA_positive_compare/esmat/survival/brca/tnbc/TCGA_BRCA"
)
...
markers_file_names <- "survival_deg_tnbc_TCGA.csv"
...
top_protective <- raw_markers[direction == "protective"][order(-abs_logHR)][
  ...
  dplyr::pull(gene)
gene_list <- list(
  "survival_TCGA_BRCA_pos_ssGSEA" = top_risk,
  "survival_TCGA_BRCA_neg_ssGSEA" = top_protective
)

# ? run ssGSEA
seurat_path <- "/home/data/sigbridger/benchmark_data/brca/TNBC"
seurat <- qs::qread(file.path(seurat_path, "tcga_tnbc_merged_seurat.qs"))
...
qs::qsave(score2, file = "ssGSEA_score_TCGA_BRCA.qs", nthreads = 4L)
