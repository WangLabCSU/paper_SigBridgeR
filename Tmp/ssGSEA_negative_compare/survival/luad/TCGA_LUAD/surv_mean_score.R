...
setwd(
  '/home/yyx/R/Project/R_code/SigBridgeR/Tmp/ssGSEA_negative_compare/survival/luad/TCGA_LUAD'
)
...
MeanScore = function(seurat_obj, es.mat, sample = "GSE", i = NULL) {
  ...
  mean_scores_scissor <- if ("scissor" %in% colnames(es.df)) {
    es.df %>%
      group_by(scissor) %>%
      summarise(across(score_row_name, mean, na.rm = TRUE)) %>%
      pivot_longer(
        cols = score_row_name,
        names_to = "cluster",
  ...
  mean_scores_degas <- if ("DEGAS" %in% colnames(es.df)) {
    es.df %>%
      group_by(DEGAS) %>%
      summarise(across(score_row_name, mean, na.rm = TRUE)) %>%
      pivot_longer(
  ...
  mean_scores_lp_sgl <- if ("LP_SGL" %in% colnames(es.df)) {
    es.df %>%
      group_by(LP_SGL) %>%
      summarise(across(score_row_name, mean, na.rm = TRUE)) %>%
      pivot_longer(
  ...
  mean_scores_pipet <- if ("PIPET" %in% colnames(es.df)) {
    es.df %>%
      group_by(PIPET) %>%
      summarise(across(score_row_name, mean, na.rm = TRUE)) %>%
      pivot_longer(
...
bubble <- ggplot2::ggplot(
  ...
  ggplot2::labs(
    ...
    subtitle = "sc-GSE123902, bulk-TCGA_LUAD",
    y = "Mean ssGSEA Score",
    x = "Group"
  ) +
...
ggplot2::ggsave(
  filename = 'Neg_ctrl_luad_TCGA_LUAD_mean_score.png',
  bubble,
  dpi = 400,
  width = 6,
  height = 6
