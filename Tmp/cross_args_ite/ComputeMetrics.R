library(data.table)
# 最终优化版本 - 内存友好的向量化操作
ComputeMetrics <- function(dt) {
  # 获取列名
  all_cols <- names(dt)
  benchmark_col <- all_cols[ncol(dt)]
  process_cols <- all_cols[2:(ncol(dt) - 1)]

  # 提取benchmark列
  benchmark_logical <- as.logical(dt[[benchmark_col]])
  ...
  # 设置行名
  rownames(result) <- c("TPR", "FPR", "TNR", "FNR", "F1", "Accuracy", "Precision", "P_pos")
