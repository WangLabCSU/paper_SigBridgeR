...
scores <- qs::qread(
  "ad/ad_Sample_100_ssgsea_score.qs",
  nthreads = 4L
)

seurat <- qs::qread(
  "/home/data/sigbridger/benchmark_binary/ad/binary_ad_GSE109887_merged_seurat.qs",
  nthreads = 4L
)
meta <- seurat[[]]
...
mean_scores <- CalcGroupMeanScores(
  ...
  sample_name = "Sample_100_GSE109887"
)

dir_out <- "binary/ad/GSE109887"

data.table::fwrite(
  mean_scores,
  file.path(dir_out, "ad_bi_100reps_neg_ctrl_stat.csv")
)
# mean_scores <- data.table::fread(file.path(
#   dir_out,
#   "ad_bi_100reps_neg_ctrl_stat.csv"
# ))
