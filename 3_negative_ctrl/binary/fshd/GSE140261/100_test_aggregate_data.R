...
scores <- qs::qread(
  "fshd/fshd_Sample_100_ssgsea_score.qs",
  nthreads = 4L
)

seurat <- qs::qread(
  "/home/data/sigbridger/benchmark_binary/fshd/binary_fshd_GSE140261_merged_seurat.qs",
  nthreads = 4L
)
meta <- seurat[[]]
if (!"SCIPAC" %in% colnames(meta)) {
  meta$SCIPAC <- meta$sig # bug compatible
}
...
screen_methods <- grepv(
  ...
  grepv("scDbl", ., invert = TRUE)
screened_label <- meta[, screen_methods]
...
mean_scores <- CalcGroupMeanScores(
  ...
  sample_name = "Sample_100_GSE140261"
)

dir_out <- "binary/fshd/GSE140261"

data.table::fwrite(
  mean_scores,
  file.path(dir_out, "fshd_bi_100reps_neg_ctrl_stat.csv")
)
# mean_scores <- data.table::fread(file.path(
#   dir_out,
#   "fshd_bi_100reps_neg_ctrl_stat.csv"
# ))
