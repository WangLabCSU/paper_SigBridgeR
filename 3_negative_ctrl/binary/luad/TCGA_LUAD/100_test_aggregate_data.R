...
seurat <- qs::qread(
  "/home/data/sigbridger/benchmark_binary/lung/binary_lung_TCGA_LUAD_merged_seurat.qs",
  nthreads = 4L
)
meta <- seurat[[]]
...
mean_scores <- CalcGroupMeanScores(
  ...
  sample_name = "Sample_100_TCGA_LUAD"
)

dir_out <- "binary/luad/TCGA_LUAD"

data.table::fwrite(
  mean_scores,
  file.path(dir_out, "luad_bi_100reps_neg_ctrl_stat.csv")
)
# mean_scores <- data.table::fread(file.path(
#   dir_out,
#   "luad_bi_100reps_neg_ctrl_stat.csv"
# ))
