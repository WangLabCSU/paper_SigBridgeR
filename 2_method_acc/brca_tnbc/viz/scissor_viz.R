...
arg_samples_with_metrics <- purrr::imap(metrics, \(dt, name) {
  ...
  if (name == "mat1") {
    arg_samples <- tibble::tibble(
      alpha = c(0.5, 0.25, 0.15, 0.1, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05), # * see log
      cutoff = cutoff
    )
  } else if (name == "mat2") {
    arg_samples <- tibble::tibble(
      alpha = c(0.55, 0.35, 0.2, 0.15, 0.1, 0.1, 0.05, 0.05, 0.05, 0.05), # * see log_2
      cutoff = cutoff
    )
  }
...
f1_bubble_heatmap <- function(
  ...
  p <- ggplot2::ggplot(
    data,
    ggplot2::aes(x = alpha, y = cutoff, fill = F1)
  ) +
