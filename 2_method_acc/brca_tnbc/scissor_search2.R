...
if (!file.exists("TCGA_BRCA_tnbc_scissor_cache2.RData")) {
  tmp <- SigBridgeR::Screen(
    ...
    alpha = 0.9,
    cutoff = 0.2,
    path2save_scissor_inputs = "TCGA_BRCA_tnbc_scissor_cache2.RData"
  )
  rm(tmp)
}
...
results <- lapply(cutoff, \(c) {
  res <- SigBridgeR::Screen(
    ...
    alpha = alpha,
    cutoff = c,
    path2load_scissor_cache = "TCGA_BRCA_tnbc_scissor_cache2.RData"
  )
...
cli::cli_alert_info("scissor_label_mat2 saved")
