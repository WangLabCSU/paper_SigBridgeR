    ...
    function(i) {
      ...
      if (imputation_i == "None") {
        scpas_result = Screen(
          ...
          sc_data = seurat,
          phenotype = surv_data,
          label_type = glue::glue("OS (year)_survival_{i}"),
...
her2_cells <- rownames(seurat[[]])
is_tumor_cell <- data.table::`%chin%`(her2_cells, tumor_cells)

scpas_random_search$benchmark = is_tumor_cell
