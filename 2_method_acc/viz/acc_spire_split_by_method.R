...
# ============================
# 螺旋图：6 个 method 各占 60°，tissue 在螺旋轴上，Accuracy 为径向值
# ============================

combined2 <- data.table::fread("combined.csv")


# 按 method_name + tissue 汇总 Accuracy 均值
spiral_acc <- combined2[, c(
  "screen_method",
  "tissue",
  "Accuracy",
  "method_name",
...
spiral_acc_by_method <- split(spiral_acc, by = "method_name")

spiral_acc_by_method <- lapply(spiral_acc_by_method, \(dt) {
  data.table::setorder(dt, sample_name, method_name)
  dt[, x := .I]
})

purrr::iwalk(
  spiral_acc_by_method,
  \(dt, dt_name) {
    # 绘制
    grDevices::png(
      glue::glue("spiral_acc_{dt_name}.png"),
      width = 14,
      height = 10,
      units = "in",
      res = 400
    ...
    # * Legend
    acc_lgd <- ComplexHeatmap::Legend(title = "Accuracy", col_fun = col_fun)
