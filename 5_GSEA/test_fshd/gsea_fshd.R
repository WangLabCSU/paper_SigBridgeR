# ! binary only (fshd has no survival phenotype)

setwd(file.path(usethis::proj_path(), "5_GSEA/test_fshd"))

library(dplyr)

# * Hyperparameter
bi_seurat_path <- "/home/data/sigbridger/benchmark_binary/fshd"
tissue <- "fshd"
mirai::daemons(3L)
...
labeled_seurat <- list.files(
  bi_seurat_path,
  pattern = glue::glue("{phenotype_class}.*\\.qs$"),
  full.names = TRUE
)
