...
data_path <- "fshd"
markers_file_names <- "binary_deg_GSE115650.csv"
...
top_protective <- raw_markers[logFC < 0][1:20] %>%
  dplyr::pull(gene)
gene_list <- list(
  "binary_GSE115650_pos_ssGSEA" = top_risk,
  "binary_GSE115650_neg_ssGSEA" = top_protective
)
...
seurat <- qs::qread(file.path(
  seurat_path,
  "binary_fshd_GSE115650_merged_seurat.qs"
))
...
qs::qsave(
  es_df,
  file = "esmat/binary/fshd/GSE115650/ssGSEA_score_GSE115650.qs",
  nthreads = 4L
)
