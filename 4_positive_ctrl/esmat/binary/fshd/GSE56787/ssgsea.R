...
data_path <- "fshd"
markers_file_names <- "binary_deg_GSE56787.csv"
...
top_protective <- raw_markers[logFC < 0][1:20] %>%
  dplyr::pull(gene)
gene_list <- list(
  "binary_GSE56787_pos_ssGSEA" = top_risk,
  "binary_GSE56787_neg_ssGSEA" = top_protective
)
...
seurat <- qs::qread(file.path(
  seurat_path,
  "binary_fshd_GSE56787_merged_seurat.qs"
))
...
qs::qsave(
  es_df,
  file = "esmat/binary/fshd/GSE56787/ssGSEA_score_GSE56787.qs",
  nthreads = 4L
)
