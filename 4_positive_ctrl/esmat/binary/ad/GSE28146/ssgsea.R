...
data_path <- "ad"
markers_file_names <- "binary_deg_GSE28146.csv"
...
top_protective <- raw_markers[logFC < 0][1:20] %>%
  dplyr::pull(gene)
gene_list <- list(
  "binary_GSE28146_pos_ssGSEA" = top_risk,
  "binary_GSE28146_neg_ssGSEA" = top_protective
)
...
seurat <- qs::qread(file.path(
  seurat_path,
  "binary_ad_GSE28146_merged_seurat.qs"
))
...
qs::qsave(
  es_df,
  file = "esmat/binary/ad/GSE28146/ssGSEA_score_GSE28146.qs",
  nthreads = 4L
)
