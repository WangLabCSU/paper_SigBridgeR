...
data_path <- "ad"
markers_file_names <- "binary_deg_GSE39420.csv"
...
top_protective <- raw_markers[logFC < 0][1:20] %>%
  dplyr::pull(gene)
gene_list <- list(
  "binary_GSE39420_pos_ssGSEA" = top_risk,
  "binary_GSE39420_neg_ssGSEA" = top_protective
)
...
seurat <- qs::qread(file.path(
  seurat_path,
  "binary_ad_GSE39420_merged_seurat.qs"
))
...
qs::qsave(
  es_df,
  file = "esmat/binary/ad/GSE39420/ssGSEA_score_GSE39420.qs",
  nthreads = 4L
)
gc()
