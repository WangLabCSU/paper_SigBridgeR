...
data.table::fwrite(
  combined,
  file = "vis/binary/ov/binary_ov_test_data.csv"
)

cli::cli_alert_success(
  "Combined test results saved to {.file binary_ov_test_data.csv}"
)
  ...
  function(ssgsea_type) {
    ...
    p <- ggplot(plot_df2) +
      ...
      labs(
        ...
        subtitle = glue::glue(
          "sc GSE165897 ; {ssgsea_type} marker \nbinary phenotype\nGSE140082: High-grade/Low-grade;\nGSE9891: Malignant/LMP
"
        )
      ) +
      theme(
...
purrr::walk(c("pos", "neg"), function(ssgsea_type) {
  file_name <- glue::glue("ov_{ssgsea_type}marker_htmap.png")
