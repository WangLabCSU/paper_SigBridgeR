...
diff_path <- "diff_test/binary/fshd"
...
data.table::fwrite(
  combined,
  file = "vis/binary/fshd/binary_fshd_test_data.csv"
)

cli::cli_alert_success(
  "Combined test results saved to {.file binary_fshd_test_data.csv}"
)
...
purrr::walk(
  ...
  function(ssgsea_type) {
    ...
    p <- ggplot(plot_df2) +
      ...
      labs(
        title = "FSHD ssGSEA Pos Ctrl",
        subtitle = glue::glue(
          "sc GSE122873 ; {ssgsea_type} marker \nbinary phenotype\nGSE115650: FSHD/Control;\nGSE140261: FSHD/Control;\nGSE56787: FSHD1+FSHD2/Control
"
        )
      ) +
      theme(
...
htmap_dir <- "vis/binary/fshd/heatmap"
...
purrr::walk(c("pos", "neg"), function(ssgsea_type) {
  file_name <- glue::glue("fshd_{ssgsea_type}marker_htmap.png")
