setwd(file.path(usethis::proj_path(), "4_positive_ctrl/vis/combined/non_tumor"))
...
available_methods <- colnames(esmats_combined)[
  colnames(esmats_combined) %in% screen_methods
]

esmats_combined_long <- data.table::melt(
  data = esmats_combined,
  measure.vars = available_methods,
...
esmats_combined_long_filtered$sc <- toupper(esmats_combined_long_filtered$sc)
label_position$sc <- toupper(label_position$sc)
